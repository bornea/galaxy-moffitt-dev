...
Takes 3 arguments: scaffold file, bait file and Y/N for a prey file"""
...
def read_scaffold(scaffold_input):
    with open(scaffold_input,'r') as x: #read in scaffold output file
        dupes = []
        for line in x:
            line = line.strip()
            temp = line.split('\t')
